...
class PX4CompanionNode(Node):
    """_summary_
    Uses to operate each drone in offboard mode
    Args:
    ...
    def __init__(self, node_name: str):
        ...
        #     DronePosition,
        #     f"{self.drone_name_}/{node_name}/stop_flight",
        #     self.stop_flight_callback_,
        # )
    ...
    def set_offboard_mode(self):
        """Set drone in OFFBOARD mode and arm with additional checks."""
        if self.is_armed_:
        ...
        for _ in range(100):
            self.publish_setpoint_()
            time.sleep(0.1)
        # Publish setpoints continuously before switching to OFFBOARD mode
        self.get_logger().info(
            "Publishing initial setpoints to prepare for OFFBOARD mode..."
        )
        time.sleep(5)
        ...
        self.get_logger().info("Verifying OFFBOARD mode...")
